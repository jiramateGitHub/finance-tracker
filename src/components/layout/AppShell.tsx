...
export function AppShell({ activeView, onChangeView, currentUserEmail, syncStatus, onLogout, children }: AppShellProps) {
  ...
  return (
    <div className="min-h-screen bg-finance-bg pb-[calc(7.5rem+env(safe-area-inset-bottom))] text-finance-text">
      <div className="mx-auto grid w-full max-w-[1320px] gap-3 px-3 py-3 sm:gap-4 sm:px-5 sm:py-4 lg:px-8">
        <Header activeItem={activeItem} currentUserEmail={currentUserEmail} syncStatus={syncStatus} onLogout={onLogout} />
        <main className="grid gap-3 pb-3 sm:gap-4">{children}</main>
      </div>

          ...
          <div className="flex items-center justify-between sm:justify-center bg-white p-1 rounded-xl border border-slate-200/90 shadow-xs w-full sm:w-auto">
            ...
            {selectedYear !== currentRealYear && (
              <button
                ...
                className="ml-1 min-h-10 sm:min-h-8 px-2.5 py-1 text-xs font-semibold rounded-lg text-indigo-700 bg-indigo-50 hover:bg-indigo-100 transition border-l border-slate-200 cursor-pointer flex items-center"
              >
                ปีปัจจุบัน
              </button>
            )}
              ...
              >
                ...
                <div className="grid gap-1.5 text-xs sm:text-sm">
                  <div className="flex justify-between items-baseline gap-2 text-emerald-700">
                    <span className="font-medium text-slate-500 text-xs shrink-0">รายรับ</span>
                    <span className="font-bold break-words tabular-nums text-right">+{formatMoney(monthTotals.income)}</span>
                  </div>
                  <div className="flex justify-between items-baseline gap-2 text-rose-700">
                    <span className="font-medium text-slate-500 text-xs shrink-0">รายจ่าย</span>
                    <span className="font-bold break-words tabular-nums text-right">-{formatMoney(monthTotals.expense)}</span>
                  </div>
                  <div className="flex justify-between items-baseline gap-2 border-t border-slate-100 pt-2 text-slate-800">
                    <span className="font-medium text-slate-600 text-xs shrink-0">คงเหลือ</span>
                    <span className={`font-extrabold break-words tabular-nums text-right ${monthTotals.balance >= 0 ? 'text-blue-700' : 'text-rose-700'}`}>
